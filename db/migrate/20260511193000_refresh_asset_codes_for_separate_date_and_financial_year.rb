class RefreshAssetCodesForSeparateDateAndFinancialYear < ActiveRecord::Migration[8.1]
  def up
    Asset.includes(:stakeholder_category, :primary_office_category, :secondary_office_category, product: :product_varieties).find_each do |asset|
      ...
      next if asset.asset_code == refreshed_asset_code
  ...
  def down
    # Asset codes intentionally remain on the refreshed split date/year format.
  end
end
